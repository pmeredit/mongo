//! An SDK for writing extension features in safe Rust.
//!
//! At the moment extensions only support registering new aggregation stages. These stages are
//! divided into 3 different types:
//! * _Desugar_ stages expand into one or more other stages.
//! * _Source_ stages generate new documents from their stage definition.
//! * _Transform_ stages accept input from another stage and may filter or mutate the document stream.
//!
//! To begin, define a new struct that implements [`AggregationStageDescriptor`] and one of
//! [`DesugarAggregationStageDescriptor`], [`SourceAggregationStageDescriptor`], or
//! [`TransformAggregationStageDescriptor`]. These descriptor objects are stateless and a registered
//! with the server using [`ExtensionPortal`]. For source and transform stages you will also need
//! to implement structs for _bound_ descriptors that are attached to a specific stage definition
//! in a pipeline as well as _executors_ that read and transform input.
//!
//! Within the implementation of a stage you should use the `bson` crate to deserialize and
//! deserialize stage definitions and documents moving through the pipeline.
//!
//! In the long run this will be a separate crate that can be pulled into a new extension project.
...
/// The error type for extension operations.
///
/// This can be used to generate errors from an extension, but may also be used to pass through
/// errors generated by the host if we call back into server code. One notable example of a host
/// error is any error that occurs when a transform stage reads from its source.
#[derive(Debug)]
pub enum Error {
...
impl Error {
    /// Create a new extension error from a code and descriptive string.
    ...
    /// Create a new extension error from a code, descriptive string, and another Rust `Error`.
    ...
    /// Take ownership of a host error.
    // TODO: this should probably pub(crate) visible.
    pub fn from_host(error: NonNull<MongoExtensionError>) -> Self {
        Self::Host(HostError(error))
    }

    /// Return the code associated with this error. May be -1 for some host errors.
...
pub mod stage_constraints {
    //! Describe constraints on where this stage may appear in an aggregation pipeline and be run.
    use serde::{Deserialize, Serialize};
...
/// Stage descriptor properties. These are independent of the stage definition and pipeline context.
#[derive(Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Pipeline context provided to aggregation stages during binding.
///
/// This is request metadata that does not appear in the stage definition itself.
///
/// During descriptor binding the context is provided as a `RawDocument`, call
/// `AggregationStageContext::try_from(context)` obtain the context during binding.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct AggregationStageContext {
    /// Name of the database this pipeline is running on.
    ...
    #[serde(rename = "$db")]
    pub db: String,
    /// Name of the collection this pipeline is running on.
    pub collection: Option<String>,
    /// UUID of the collection this pipeline is running on.
    #[serde(rename = "collectionUUID")]
    pub collection_uuid: Option<Uuid>,
    /// If true, this pipeline is begin run in a routing context rather that on the data plane.
    pub in_router: bool,
...
pub trait AggregationStageDescriptor {
    ...
    ///
    /// This value may be cached by callers including the host so calls should be idempotent.
    fn properties(&self) -> AggregationStageProperties;
}

/// A de-sugaring stage descriptor.
///
/// De-sugaring stages expand to one or more other aggregation stages but do not have a concrete
/// implementation of their own. Implementers need to be careful to avoid expansions that generate
/// cycles or otherwise do not terminate.
...
/// A source stage descriptor.
///
/// Source stages do not accept input from any other stage, instead they generate new documents
/// from the stage definition. Stages that read foreign data sources will typically be of this
/// type.
...
/// A transform stage descriptor.
///
/// Transform stages accept input from another stage and transform the document stream. This could
/// be any type of transform: filtering, reordering, or document-by-document transforms.
...
/// A source descriptor bound to stage definition and pipeline context.
///
/// These descriptor types are used during query planning and optimization. They may also be used
/// to create executors when needed.
...
/// A transform descriptor bound to stage definition and pipeline context.
///
/// These descriptor types are used during query planning and optimization. They may also be used
/// to create executors when needed.
pub trait TransformBoundAggregationStageDescriptor {
    ...
    /// Create a new executor based on bound state from creation, with an input source to read from.
    fn create_executor(
        &self,
...
/// The result of successful [`AggregationStageExecutor::get_next`] call.
///
/// In the average case this will either a BSON `RawDocument` or a terminating `EOF`. In other cases
/// callers should typically pass the state through without modifying it.
...
/// `ExtensionPortal` is used to register hooks into server functionality.
///
/// At the moment this is used mostly to register new aggregation stages. Stage descriptors are
/// typically stateless so these calls will look like `portal.register_desugar_aggregation_stage(MyStageType)`.
...
impl ExtensionPortal {
    ...
    /// Install any services provided by the host.
    pub fn install_host_services(&self) {
        ExtensionHostServices::install(unsafe { self.0.as_ref().hostServices })
    }

    /// Register a new desugaring stage, providing an instance of the stage descriptor.
    ...
    /// Register a new source stage, providing an instance of the stage descriptor.
    ...
    /// Register a new transform stage, providing an instance of the stage descriptor.
...
/// Services provided by the host server to extensions.
///
/// These services are typically unconnected to an particular request, and are install on
...
/// Generates file:line as a valid c-string. For use with idle thread host services.
#[macro_export]
