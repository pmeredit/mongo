...
namespace {
    ...
    int c_plugin_echo_aggregation_stage_get_next(mongodb_aggregation_stage* stage, char** result, size_t* result_len) {
    ...
    void c_plugin_echo_aggregation_stage_close(mongodb_aggregation_stage* stage) {
        ...
        auto ptr = std::unique_ptr<c_plugin_echo_aggregation_stage>((c_plugin_echo_aggregation_stage*)stage);
        ptr = nullptr;
    }

    int c_plugin_echo_aggregation_stage_parse(char bson_type, char* bson_value, size_t bson_value_len, mongodb_aggregation_stage** stage, char **error, size_t* error_len) {
        ...
        *stage = (mongodb_aggregation_stage*)echo_stage.release();  // leak the value to the caller as a mongodb_aggregation_stage*.
        *error = nullptr;
        *error_len = 0;
        return 0;
    }
...
int mongodb_initialize_plugin(mongodb_plugin_portal* plugin_portal, char** error, size_t* error_len) {
    return plugin_portal->add_aggregation_stage(c_plugin_echo_aggregation_stage::kStageName, sizeof(c_plugin_echo_aggregation_stage::kStageName), c_plugin_echo_aggregation_stage_parse, error, error_len);
}
