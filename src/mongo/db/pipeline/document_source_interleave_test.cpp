...
public:
    void setUp() override {
        ...
        settings.setReplSetString("interleaveTestSet/node1:12345");
...
// A 'let' variable defined in a $interleave stage is expected to be available to all sub-pipelines. For
// sub-pipelines below the immediate one, they are passed to via ExpressionContext. This test
// confirms that variables defined in the ExpressionContext are captured by the $interleave stage.
TEST_F(DocumentSourceLookUpTest, PreservesParentPipelineLetVariables) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        BSON("$interleave" << BSON("from"
            ...
            .firstElement(),
        expCtx);
    auto interleaveStage = static_cast<DocumentSourceLookUp*>(docSource.get());
    ASSERT(interleaveStage);

    ASSERT_EQ(varId, interleaveStage->getVariablesParseState_forTest().getVariable("foo"));
    ASSERT_VALUE_EQ(Value(123), interleaveStage->getVariables_forTest().getValue(varId, Document()));
}
...
TEST_F(DocumentSourceLookUpTest, ShouldTruncateOutputSortOnAsField) {
    ...
    auto interleave = DocumentSourceLookUp::createFromBson(Document{{"$interleave",
                                                                 Document{{"from", "a"_sd},
                                                                          {"localField", "b"_sd},
                                                                          {"foreignField", "c"_sd},
                                                                          {"as", "d.e"_sd}}}}
                                                           ...
                                                           .firstElement(),
                                                       expCtx);
    interleave->setSource(source.get());

    BSONObjSet outputSort = interleave->getOutputSorts();
...
TEST_F(DocumentSourceLookUpTest, AcceptsPipelineSyntax) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        BSON("$interleave" << BSON("from"
            ...
            .firstElement(),
        expCtx);
    auto interleave = static_cast<DocumentSourceLookUp*>(docSource.get());
    ASSERT_TRUE(interleave->wasConstructedWithPipelineSyntax());
}
...
TEST_F(ReplDocumentSourceLookUpTest, RejectsPipelineWithChangeStreamStage) {
    ...
    // Verify that attempting to create a $interleave pipeline containing a $changeStream stage fails.
    ASSERT_THROWS_CODE(
        DocumentSourceLookUp::createFromBson(
            fromjson("{$interleave: {from: 'coll', as: 'as', pipeline: [{$changeStream: {}}]}}")
                .firstElement(),
            expCtx),
        AssertionException,
...
TEST_F(ReplDocumentSourceLookUpTest, RejectsSubPipelineWithChangeStreamStage) {
    ...
    // Verify that attempting to create a sub-$interleave pipeline containing a $changeStream stage
    // fails at parse time, even if the outer pipeline does not have a $changeStream stage.
    ASSERT_THROWS_CODE(
        DocumentSourceLookUp::createFromBson(
            fromjson("{$interleave: {from: 'coll', as: 'as', pipeline: [{$match: {_id: 1}}, {$interleave: "
...
TEST_F(DocumentSourceLookUpTest, RejectsLocalFieldForeignFieldWhenPipelineIsSpecified) {
    ...
    try {
        auto interleaveStage = DocumentSourceLookUp::createFromBson(
            BSON("$interleave" << BSON("from"
                                   << "coll"
                                   << "pipeline"
                                   << BSON_ARRAY(BSON("$match" << BSON("x" << 1)))
...
TEST_F(DocumentSourceLookUpTest, RejectsLocalFieldForeignFieldWhenLetIsSpecified) {
    ...
    ASSERT_THROWS_CODE(DocumentSourceLookUp::createFromBson(BSON("$interleave" << BSON("from"
                                                                                   << "coll"
                                                                                   << "let"
                                                                                   << BSON("var1"
                                                                                           << "$a")
...
TEST_F(DocumentSourceLookUpTest, ShouldBeAbleToReParseSerializedStage) {
    ...
    //
    // Serialize the $interleave stage and confirm contents.
    //
    vector<Value> serialization;
    interleaveStage->serializeToArray(serialization);
    ...
    ASSERT_EQ(serializedDoc["$interleave"].getType(), BSONType::Object);

    auto serializedStage = serializedDoc["$interleave"].getDocument();
    ...
    //
    // Create a new $interleave stage from the serialization. Serialize the new stage and confirm that
...
TEST_F(DocumentSourceLookUpTest, ShouldPropagatePauses) {
    ...
    // Set up the $interleave stage.
    auto interleaveSpec = Document{{"$interleave",
                                Document{{"from", fromNs.coll()},
                                         {"localField", "foreignId"_sd},
                                         {"foreignField", "_id"_sd},
                                         {"as", "foreignDocs"_sd}}}}
                          .toBson();
    auto parsed = DocumentSourceLookUp::createFromBson(interleaveSpec.firstElement(), expCtx);
    auto interleave = static_cast<DocumentSourceLookUp*>(parsed.get());
    ...
    interleave->setSource(mockLocalSource.get());
    ...
    ASSERT_TRUE(interleave->getNext().isEOF());
    ASSERT_TRUE(interleave->getNext().isEOF());
    interleave->dispose();
}
...
TEST_F(DocumentSourceLookUpTest, ShouldPropagatePausesWhileUnwinding) {
    ...
    interleave->setUnwindStage(DocumentSourceUnwind::create(
...
TEST_F(DocumentSourceLookUpTest, LookupReportsAsFieldIsModified) {
    ...
    auto modifiedPaths = interleave->getModifiedPaths();
...
TEST_F(DocumentSourceLookUpTest, ShouldCacheNonCorrelatedSubPipelinePrefix) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {x:1}}, {$sort: {x: 1}}, "
    ...
    auto subPipeline = interleaveStage->getSubPipeline_forTest(DOC("_id" << 5));
    ASSERT(subPipeline);
...
TEST_F(DocumentSourceLookUpTest,
    ...
    // identified as correlated and the cache is placed before it in the $interleave pipeline.
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {x:1}}, {$sort: {x: 1}}, "
...
TEST_F(DocumentSourceLookUpTest, ExprEmbeddedInMatchExpressionShouldBeOptimized) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {$expr: {$eq: "
...
TEST_F(DocumentSourceLookUpTest,
    ...
    // The $project stage defines a local variable with the same name as the $interleave 'let' variable.
...
TEST_F(DocumentSourceLookUpTest, ShouldInsertCacheBeforeCorrelatedNestedLookup) {
    ...
    // Create a $interleave stage whose pipeline contains nested $interleaves. The third-level $interleave
    // refers to a 'let' variable defined in the top-level $interleave. Verify that the second-level
    // $interleave is correctly identified as a correlated stage and the cache is placed before it.
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {from: 'coll', as: 'as', let: {var1: '$_id'}, pipeline: [{$match: "
                 "{x:1}}, {$sort: {x: 1}}, {$interleave: {from: 'coll', as: 'subas', pipeline: "
                 "[{$match: {x: 1}}, {$interleave: {from: 'coll', as: 'subsubas', pipeline: [{$match: "
    ...
    auto expectedPipe = fromjson(
        str::stream() << "[{mock: {}}, {$match: {x:{$eq: 1}}}, {$sort: {sortKey: {x: 1}}}, "
                      << sequentialCacheStageObj()
                      << ", {$interleave: {from: 'coll', as: 'subas', let: {}, pipeline: "
                         "[{$match: {x: 1}}, {$interleave: {from: 'coll', as: 'subsubas', "
...
TEST_F(DocumentSourceLookUpTest,
    ...
    // The nested $interleave stage defines a 'let' variable with the same name as the top-level 'let'.
    // Verify the nested $interleave is identified as non-correlated and the cache is placed after it.
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {x:1}}, {$sort: {x: 1}}, "
                 "{$interleave: {let: {var1: '$y'}, pipeline: [{$match: {$expr: { $eq: ['$z', "
    ...
    auto expectedPipe = fromjson(
        str::stream() << "[{mock: {}}, {$match: {x:{$eq: 1}}}, {$sort: {sortKey: {x: 1}}}, "
                         "{$interleave: {from: 'coll', as: 'subas', let: {var1: '$y'}, "
...
TEST_F(DocumentSourceLookUpTest, ShouldCacheEntirePipelineIfNonCorrelated) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {}, pipeline: [{$match: {x:1}}, {$sort: {x: 1}}, {$interleave: "
    ...
    auto expectedPipe = fromjson(
        str::stream()
        << "[{mock: {}}, {$match: {x:{$eq: 1}}}, {$sort: {sortKey: {x: 1}}}, {$interleave: {from: "
...
TEST_F(DocumentSourceLookUpTest,
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson(
            "{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {x: {$gte: 0}}}, {$sort: {x: "
    ...
    interleaveStage->setSource(mockLocalSource.get());
    ...
    auto subPipeline = interleaveStage->getSubPipeline_forTest(DOC("_id" << 0));
    ASSERT(subPipeline);
    ...
    // Verify the first result (non-cached) from the $interleave, for local document {_id: 0}.
    auto nonCachedResult = interleaveStage->getNext();
    ...
    subPipeline = interleaveStage->getSubPipeline_forTest(DOC("_id" << 1));
    ASSERT(subPipeline);
    ...
    auto cachedResult = interleaveStage->getNext();
    ...
    cachedResult = interleaveStage->getNext();
    ASSERT(cachedResult.isAdvanced());
    ASSERT_DOCUMENT_EQ(
        Document{fromjson(
...
TEST_F(DocumentSourceLookUpTest,
    ...
    auto firstResult = interleaveStage->getNext();
    ASSERT_DOCUMENT_EQ(
    ...
    auto secondResult = interleaveStage->getNext();
...
TEST_F(DocumentSourceLookUpTest, ShouldNotCacheIfCorrelatedStageIsAbsorbedIntoPlanExecutor) {
    ...
    auto docSource = DocumentSourceLookUp::createFromBson(
        fromjson("{$interleave: {let: {var1: '$_id'}, pipeline: [{$match: {$expr: { $gte: ['$x', "
