...
def configure(conf, env):
    ...
    if env.TargetOSIs("windows"):
        ...
        redist_path = env['MSVS'].get('VCREDISTPATH', None)
        redist_file = env['MSVS'].get('VCREDISTEXE', None)

        # I'd love to pass the files through as variables with $, but I don't
        # know if ARCHIVE_ADDITIONS stuff substitutes. This will go away with
        # hygienic builds.
        if not (redist_path and redist_file):
            env.FatalError('Required CRT redistributable not found; cannot build distribution packages')

        env.Append(ARCHIVE_ADDITIONS=[redist_file])
        env.Append(ARCHIVE_ADDITION_DIR_MAP={
            redist_path : "bin"
        })
