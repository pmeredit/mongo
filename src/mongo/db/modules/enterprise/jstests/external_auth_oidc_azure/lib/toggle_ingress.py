...
def valid_json_response_or_throw(response, expected_code=200):
    ...
    if response.status_code != expected_code:
        logging.warn('Error calling %s with unexpected status code %s : %s',
...
def validate_container_ingress_enabled(container_app_expected, container_app_response):
    ...
    if len(allowed_ips) != 1 or allowed_ips[0]['ipAddressRange'] != expected_allowed_ips[0]['ipAddressRange']:
        logging.warning(
            'Allowed IPs requested have not been updated! Sent %s but received %s after patch: ', 
                str(container_app_expected.ip_security_restrictions()), str(allowed_ips))
        sys.exit(1)
    ...
    if int(container_app_response.exposed_port()) != int(container_app_expected.exposed_port()):
        logging.warning('Remote ingress port was not updated! Expected %s but got %s',
                        container_app_response.exposed_port(), container_app_expected.exposed_port())
        sys.exit(1)

    logging.debug('Ingress enabled on app container, listening at %s:%s',
                container_app_response.hostname(), container_app_response.exposed_port())
...
def validate_container_ingress_disabled(container_app_expected, container_app_response):
    ...
    if container_app_response.ingress() is not None:
        logging.warning('Ingress rules are still defined for the container app after disabling! Please check the configuration! App still open for %s', str(
            container_app_expected.ingress()))
        sys.exit(1)
...
class TokenService:
    ...
    def get_auth_token(self):
        ...
        r = requests.post(
            'https://login.microsoftonline.com/{}/oauth2/v2.0/token'.format(self.config['oidc_azure_tenant_id']), data=payload)
    ...
    def enable_ingress(self, for_ip, for_port):
        self._configuration()['ingress'] = {
            ...
            'ipSecurityRestrictions' : [{
                'ipAddressRange': '{}/32'.format(for_ip),
                'action': 'Allow'
            }]
        }
...
class ContainerAppService:
    ...
    def update_container_app(self, container_app, validator_fn):
        ...
        patch_response = requests.patch(container_app.resource_uri,
...
class ContainerAppRevisionService:
    ...
    def get_active_revision_name(self, resource):
        ...
        active_revisions = list(filter(lambda v: v['properties']['active'] is True, revision_json['value']))
...
def enable_ingress(container_app_service, token_service, resource_formatter, container_app, exposed_port):
    container_app.enable_ingress(get_my_ip(), exposed_port)
    container_app = container_app_service.update_container_app(container_app, validate_container_ingress_enabled)
    
    con_app_revision_service = ContainerAppRevisionService(token_service, resource_formatter)

    active_revision = con_app_revision_service.get_active_revision_name(resource_formatter.format_revision_list_resource_uri())
...
def main():
    ...
    if args.mode == 'enable':
        container_app = enable_ingress(container_app_service, token_service, resource_formatter, container_app, config['oidc_azure_container_port'])
    else:
