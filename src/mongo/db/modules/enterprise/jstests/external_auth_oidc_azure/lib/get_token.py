...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        prog='Azure Container Token-Getter Script')
    ...
    if not hostname:
        print(
            'Could not find hostname file for the app container at path args.hostname_file!')
        sys.exit(1)
    ...
    print("Getting token from {}@{}:{}, object_id={}, api_version={}, resource={}".format(user,
          hostname, str(port), object_id, api_version, resource))
    ...
    process = subprocess.run(['ssh', '-i', key_file, '-o', 'StrictHostKeyChecking=no', '-p', port, '{}@{}'.format(user, hostname), 'curl', '-s', '{}:{}?resource={}\&object_id={}\&api-version={}'.format(CONST_REMOTE_URL, CONST_REMOTE_PORT, resource, object_id, api_version)],
