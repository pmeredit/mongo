...
testLargeAccumulator(
    ...
    () => {
        ...
        let memLimit = 250 * 1024 * 1024;
        assert.commandWorked(db.adminCommand({setParameter: 1, internalQueryMaxPushBytes: memLimit}));
...
testLargeAccumulator(
    ...
    () => {
        ...
        // update the memory limit for the accumulator
        let memLimit = 2 * 1024 * 1024 * 1024 - 1;
        assert.commandWorked(db.adminCommand({setParameter: 1, internalQueryMaxPushBytes: memLimit}));

        const seed = Array(100 * 1024 + 1).toString();
        inputColl.insert({_id: 1, ts: 1, docCount: 200, docSize: 90, seed: seed}); // will generate 200 - 9MB documents
