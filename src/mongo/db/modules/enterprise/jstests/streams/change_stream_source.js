...
function runChangeStreamTestSplitLargeEvent(
    {sourceConfig, inputMessageCount, outputMessageCount, dlqMessageCount, errorMessage}) {
    const dbName = "test";
    const inputCollName = "testin";
    const outputCollName = "testout";
    const dlqCollName = "dlqColl";
    ...
    outputColl.drop();
    dlqColl.drop();
    const processorName = "changeStreamSourceLargeEvent";

    inputColl.insert({_id: 10000, a: 1, largeField: "a".repeat(16 * 1024 * 1024 - 100)});
    inputColl.insert({_id: 10001, a: 2, largeField: "a".repeat(16 * 1024 * 1024 - 46)});
    inputColl.insert({_id: 10002, a: 1, largeField: "a".repeat(16 * 1024 * 1024 - 100)});
    inputColl.insert({_id: 10003, a: 1, largeField: "a".repeat(16 * 1024 * 1024 - 46)});
    inputColl.insert({_id: 10004, a: 1, largeField: "a".repeat(16 * 1024 * 1024 - 46)});
    inputColl.insert({_id: 10005, a: 2, largeField: "a".repeat(16 * 1024 * 1024 - 100)});
    db.runCommand({collMod: inputCollName, changeStreamPreAndPostImages: {enabled: true}});
    let startCmd = {
        streams_startStreamProcessor: '',
        name: processorName,
    ...
    let res = db.runCommand(startCmd);
    if (errorMessage) {
        assert.commandFailed(res);
        assert(res["errmsg"].includes(errorMessage));
        return;
    }
    const verboseStats = getStats(processorName);
    assert.eq(verboseStats["ok"], 1);
    ...
    inputColl.insert(
        Array.from({length: 10}, (_, i) => ({_id: i, a: 1, smallField: "b".repeat(i)})));
    inputColl.updateOne({_id: 10000}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 100)}});
    inputColl.updateOne({_id: 10001}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 46)}});
    inputColl.updateOne({_id: 10002}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 100)}});
    inputColl.insert(
        Array.from({length: 10}, (_, i) => ({_id: 10 + i, a: 2, smallField: "b".repeat(i)})));
    inputColl.updateOne({_id: 10003}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 46)}});
    inputColl.updateOne({_id: 10004}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 46)}});
    inputColl.insert({_id: 10006, a: 3, largeField: "a".repeat(16 * 1024 * 1024 - 100)});
    inputColl.updateOne({_id: 10005}, {$set: {largeField: "d".repeat(16 * 1024 * 1024 - 100)}});
    inputColl.insert(
        Array.from({length: 10}, (_, i) => ({_id: 20 + i, a: 3, smallField: "c".repeat(i)})));

    assert.soon(() => {
        const operatorStats = getStats(processorName)['operatorStats'];
        return operatorStats[0]['inputMessageCount'] == inputMessageCount &&
            operatorStats[2]["outputMessageCount"] == outputMessageCount &&
            operatorStats[2]["dlqMessageCount"] == dlqMessageCount;
    });

    const dlqDocs = dlqColl.find({}).toArray();
    for (const dlqDoc of dlqDocs) {
        assert(dlqDoc.errInfo.reason.includes("Document to upsert is larger than 16777216"));
        assert.eq(dlqDoc.operatorName, "MergeOperator");
    }
    jsTestLog(getStats(processorName));
    stopStreamProcessor(processorName);
}

runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
        fullDocument: "required",
        fullDocumentBeforeChange: "required",
    },
    inputMessageCount: 37,
    outputMessageCount: 37,
    dlqMessageCount: 0
});
runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
...
runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
        fullDocument: "required",
        fullDocumentBeforeChange: "required",
        pipeline: [{$match: {"fullDocument.a": 2}}]
    },
    inputMessageCount: 11,
    outputMessageCount: 11,
    dlqMessageCount: 0
});
runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
        fullDocument: "required",
        fullDocumentBeforeChange: "required",
        pipeline: [{$match: {"fullDocument.a": 1}}]
    },
    inputMessageCount: 14,
    outputMessageCount: 14,
    dlqMessageCount: 0
});
runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
        fullDocument: "required",
        fullDocumentBeforeChange: "required",
        pipeline: [{$match: {"fullDocument.a": 3}}]
    },
    inputMessageCount: 11,
    outputMessageCount: 11,
    dlqMessageCount: 0
...
runChangeStreamTestSplitLargeEvent({
    sourceConfig: {
        ...
        pipeline: [
            ...
            {$match: {"fullDocument.a": 3}},
        ]
    },
    errorMessage: "$changeStreamSplitLargeEvent must be the last stage in the pipeline"
});
