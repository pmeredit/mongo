...
const runLateDocumentsTest = ({connectionRegistry = [], $source, groupID, insert}) => {
    ...
    jsTestLog(stats2);
    assert.eq(stats2['inputMessageCount'], documents.length + lateDocuments.length);
    assert.eq(stats2['outputMessageCount'], documents.length - 1);
    assert.eq(lateDocuments.length, stats2['dlqMessageCount']);
