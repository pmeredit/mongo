...
function check_pre_image(res, expected_pre_image, return_mode) {
    if (return_mode == undefined) {
        return;
    }
    if (expected_pre_image == undefined) {
        assert(!res.cursor.firstBatch[0].value);
        return;
    }
    for (let key of Object.keys(expected_pre_image)) {
        assert.eq(res.cursor.firstBatch[0]["value"][key], expected_pre_image[key]);
    }
}

function testHelper(return_mode) {
...

    {
        ...
        // This is an upsert with no matching document.
        check_pre_image(res, undefined, return_mode);
...

    {
        ...
        res = assert.commandWorked(
            edb.runCommand({"insert": "basic", documents: [expected_pre_image]}));
        assert.eq(res.n, 1);
...

    {
        ...
        expected_pre_image =
            {"_id": 1, "first": "dwayne", "middle": "E", "aka": "president camacho"};
        check_pre_image(res, expected_pre_image, return_mode);
    ...
    {
        print(
            "Update a document with fields that get encrypted and collation (used in the filter)");
        let error = assert.throws(() => edb.adminCommand({
            bulkWrite: 1,
            ops: [{
                update: 0,
