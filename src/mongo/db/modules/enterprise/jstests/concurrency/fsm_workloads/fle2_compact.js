...
var $config = (function() {
    ...
    var states = {
        ...
        compact: function compact(db, collName) {
            ...
            for (let i = 0; i < 3; i++) {
                assertWhenOwnColl.writeOK(encryptedColl.insert(insertDoc));
            }
            assertWhenOwnColl.commandWorked(encryptedColl.compact());
            this.compactRuns++;
        },
    };
    ...
    function teardown(db, collName, cluster) {
        ...
        res = ecoll.find({testData: 1}).toArray();
        assertWhenOwnColl.eq(res.length, this.threadCount);
