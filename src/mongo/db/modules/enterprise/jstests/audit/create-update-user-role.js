...
assert.commandWorked(
    db.runCommand({createUser: "user2", pwd: "user", roles: [{role: "readWrite", db: "admin"}]}));
audit.assertEntryRelaxed("authCheck", {
    args: {createUser: "user2", pwd: "xxx", $db: "test", roles: [{role: "readWrite", db: "admin"}]}
});
audit.assertEntry("createUser",
                  {user: "user2", db: "test", roles: [{role: "readWrite", db: "admin"}]});

assert.commandWorked(
    db.runCommand({createUser: "user3", pwd: "user", roles: [], authenticationRestrictions: []}));
audit.assertEntryRelaxed("authCheck",
...
audit.assertEntry(
    "updateUser",
    {user: "user1", db: "test", passwordChanged: false, roles: [{role: "readWrite", db: "admin"}]});
...
audit.assertEntryRelaxed("authCheck", {
    args: {createRole: "role2", $db: "test", privileges: [], roles: [{role: "role1", db: "test"}]}
});
audit.assertEntry(
    "createRole",
...
assert.commandWorked(db.runCommand({updateRole: "role3", roles: [{role: "role1", db: "test"}]}));
audit.assertEntryRelaxed(
    "authCheck", {args: {updateRole: "role3", $db: "test", roles: [{role: "role1", db: "test"}]}});
audit.assertEntry("updateRole", {role: "role3", db: "test", roles: [{role: "role1", db: "test"}]});

assert.commandWorked(db.runCommand({updateRole: "role1", privileges: []}));
audit.assertEntryRelaxed("authCheck", {args: {updateRole: "role1", $db: "test", privileges: []}});
audit.assertEntry("updateRole", {role: "role1", db: "test"});
