...
def main():
    arg_parser = argparse.ArgumentParser(
        prog='GCP VM ID Token Acquisition Script')
    ...
    if not vm_external_ip:
        print(
            'Could not retrieve GCE VM external IP from file at path args.vm_info_file!')
        sys.exit(1)

    user = "evergreen"
    audience = config.get('audience')

    print("Getting token from {}@{}, audience={}".format(user,
          vm_external_ip, audience))
    ...
    process = subprocess.run(['ssh', '-v', '-v', '-v', '-i', ssh_key_file_name, '-o', 'StrictHostKeyChecking=no', '{}@{}'.format(user, vm_external_ip), 'curl', '-s', '-H', '"Metadata-Flavor: Google"', '"http://metadata/computeMetadata/v1/instance/service-accounts/default/identity?audience={}"'.format(audience)],
    ...
    try:
        ...
        _ = jwt.decode(jwt=token_str, options={'verify_signature': False, 'require': ['exp', 'iss', 'sub', 'aud']})
