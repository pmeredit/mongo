...
def read_config_from_file(config_file_name):
    ...
    if 'projectID' not in config:
        logging.warning(
            'Project ID not specified as part of config')
        sys.exit(1)

    if 'zone' not in config:
        logging.warning(
            'Project zone not specified as part of config')
        sys.exit(1)

    if 'instance_template_url' not in config:
        logging.warning(
            'Instance template URL not specified as part of config')
        sys.exit(1)
    ...
    for zone, response in agg_list:
        if response.instances:
            ...
            for instance in response.instances:
                print(f" - {instance.name} ({instance.network_interfaces[0].access_configs[0].nat_i_p})")
...
def create_instance(instance_client, project_id, zone, instance_template, gce_vm_info_file_name):
    ...
    if operation.error_code:
        logging.warn(f"Error when creating the VM instance {instance_name}: [Code: {operation.error_code}] - {operation.error_message}")
        sys.exit(1)

    logging.info(f"Instance {instance_name} created, writing host information to {gce_vm_info_file_name}")
...
def delete_instance(instance_client, gce_vm_info_file_name, project_id, zone):
    ...
    operation = instance_client.delete(
        project=project_id, zone=zone, instance=gce_vm_info['name']
    )
    operation.result(timeout=300)
    if operation.error_code:
        logging.warn(f"Error when deleting the VM instance {gce_vm_info['name']}: [Code: {operation.error_code}] - {operation.error_message}")
        sys.exit(1)

    logging.info(f"Instance {instance_name} deleted, now removing GCE VM info file ({gce_vm_info_file_name})")
...
def main():
    ...
    if args.mode == 'create':
        create_instance(instance_client, config['projectID'], config['zone'], config['instance_template_url'], args.output_file)
