...
function runTest(insertHigherTermOplogEntry) {
    ...
    shardingRestoreTest.getShardRestoreTests().forEach((magicRestoreTest) => {
        magicRestoreTest.rst.nodes.forEach((node) => {
            ...
            // Includes the 4 addOrRemoveShardInProgress entries generated by
            // transitionFromDedicatedConfigServer.
            assert.eq(entriesAfterBackup.length,
                      4,
                      `entriesAfterBackup = ${tojson(entriesAfterBackup)} is not of length 4`);
        });
    });
    ...
    // Only shardedClusterCardinalityForDirectConns should remain.
    let docs = primary.getDB("config").getCollection("clusterParameters").find().toArray();
    assert.eq(docs.length, 1);
    assert.eq(docs[0]._id, "shardedClusterCardinalityForDirectConns");
