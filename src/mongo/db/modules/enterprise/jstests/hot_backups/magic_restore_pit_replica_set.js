...
// cursor was opened. We will pass these oplog entries to magic restore to perform a PIT restore, so
// they will be reinserted and reflected in the final state of the data.
['e', 'f', 'g', 'h'].forEach(
    key => { assert.commandWorked(sourceDb.getCollection(coll).insert({[key]: 1})); });
...
const restoredConfig = assert.commandWorked(destPrimary.adminCommand({replSetGetConfig: 1})).config;
...
// TODO SERVER-82686: Once we take a stable checkpoint at the end of magic restore, the oldest
// timestamp on the restored node should be the top of the oplog. We should test that reads at this
// timestamp succeed. As of now, since we take an unstable checkpoint at the end of magic restore,
// the oldest timestamp on restart is unset. This results in the following read failing, but not
// for the correct reason.
// A restored node will not preserve any history. The oldest timestamp should be set to the top of
// the oplog during recovery oplog application.
