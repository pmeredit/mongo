...
const consumeBackupCursor = function(backupCursor, dest) {
    ...
    while (backupCursor.hasNext()) {
        ...
        // files.
        if (!_isTableFile(backupItem.filename)) {
            if (pathExists(destFilePath)) {
                ...
                jsTestLog(`Removing existing non-table file ${
                    destFilePath} in preparation of copying a newer version of it`);
                removeFile(destFilePath);
            }

            jsTestLog(`Copying non-table file from ${backupItem.filename} to ${destFilePath}`);
            assert.eq(backupItem.length, backupItem.fileSize);
            assert.eq(backupItem.offset, 0);
            copyFile(backupItem.filename, destFilePath);
            continue;
        }
...
/**
 * Returns true if the file in 'path' is a table.
 */
const _isTableFile = function(path) {
    return path.endsWith(".wt");
};
