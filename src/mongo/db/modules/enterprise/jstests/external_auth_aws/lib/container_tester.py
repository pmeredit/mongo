...
def remote_ps_container(cluster):
    ...
    for task in task_list["tasks"]:
        ...
        print(
            "{:<43}{:<9}{:<25}{:<25}{:<16}".format(
                task_id, lastStatus, public_ip, private_ip_address, taskDefinition_short
            )
        )
...
def _remote_create_container_args(args):
    ...
    remote_create_container(
        args.cluster, args.task_definition, args.service, args.subnets, args.security_group, 1
    )


def remote_create_container(
    cluster, task_definition, service_name, subnets, security_group, retry_count
):
    """
...
def _run_e2e_test(script, files, cluster, task_definition, subnets, security_group):
    ...
    remote_create_container(
        ...
        subnets,
        security_group,
        DEFAULT_ECS_CREATE_SERVICE_RETRY,
    )
...
def main() -> None:
    ...
    if os.getenv("LSAN_OPTIONS") or os.getenv("ASAN_OPTIONS"):
        ...
        print(
            "WARNING: LSAN/ASAN is not supported in ECS containers due to the missing support for ptrace"
        )
        return
