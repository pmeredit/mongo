...
protected:
    ...
    auto checkAnalysis(const char* inputJSON, const char* outputJSON) {
        ...
        uint32_t bson_len = 0;
        auto ret = mongo_csfle_v1_analyze_query(
            matcher, toBSONForAPI(inputJSON).first, "db.test", 7, &bson_len, nullptr);
        ON_BLOCK_EXIT([ret] { mongo_csfle_v1_bson_free(ret); });
        auto sret = fromBSONForAPI(ret);

        ASSERT_EQ(sret, outputJSON);
