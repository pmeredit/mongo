...
void exitSubtree(const ExpressionContext& expCtx, std::stack<Subtree>& subtreeStack) {
    if (auto compared = std::get_if<Subtree::Compared>(&subtreeStack.top().output))
        if (auto encrypted = std::get_if<Subtree::Compared::Encrypted>(&compared->state))
...
[[noreturn]] decltype(Subtree::Compared::state) uassertedEncryptedEncryptedMismatch(
    const FieldPath& currentField,
    const std::vector<std::reference_wrapper<const FieldPath>>& comparedFields) {
    uasserted(31100,
...
decltype(Subtree::Compared::state) attemptReconcilingAgainstEncryption(
    ...
    if (auto encryptedType = getEncryptionTypeForPathEnsureNotPrefix(schema, fieldPath))
        ...
        return encryptedType == currentEncryptedType
            ? Subtree::Compared::Encrypted{currentEncryptedType}
        ...
        [&](auto&& state) -> decltype(Subtree::Compared::state) {
            ...
            if
                constexpr(std::is_same_v<
                          StateType,
                          Subtree::Compared::
                              Unknown>) return reconcileAgainstUnknownEncryption(schema, fieldPath);
            else if
                constexpr(
                    std::is_same_v<
                        StateType,
                        Subtree::Compared::
                            NotEncrypted>) return attemptReconcilingAgainstNoEncryption(schema,
                                                                                        fieldPath,
                                                                                        compared
                                                                                            ->fields,
                                                                                        compared
                ...
                constexpr(
                    std::is_same_v<
                        StateType,
                        Subtree::Compared::
                            Encrypted>) return attemptReconcilingAgainstEncryption(schema,
                                                                                   fieldPath,
                                                                                   compared->fields,
                                                                                   state.type);
        },
    ...
    if (auto compared = std::get_if<Subtree::Compared>(&subtreeStack.top().output)) {
        std::visit(
            [&](auto&& state) {
                using StateType = std::decay_t<decltype(state)>;
                if
                    constexpr(!std::is_same_v<StateType, Subtree::Compared::Unknown> &&
...
/**
 * We prefer front-loading work and doing as much as possible in the PreVisitor for organization.
 */
...
private:
    ...
    void visit(ExpressionIn*) final {
        // This isn't going to work at the moment because we cannot descend into arrays in a
        // Compared context but there is no harm in setting it up.
        enterSubtree(Subtree::Compared{}, subtreeStack);
    }
