...
/**
 * Minimal implementation of ReplicationCoordinator, designed to allow mongocryptd to internally
 * identify itself as a single-node replica set. When set on the cryptd's service context, this
 * class allows (for instance) a $changeStream aggregation to be correctly parsed and processed by
 * the encryption engine. If a replication coordinator is not set, then $changeStream will think
 * that the cryptd is a standalone mongod - even if the underlying deployment is indeed a replica
 * set. Instead of the expected error stating that $changeStream is not supported for automatic
 * encryption, this will produce a non-sequitur error message stating that $changeStream can only
 * run on a replica set. This violates our intent that the mongocryptd should be transparent to the
 * end-user.
 */
class CryptDReplicationCoordinatorNoOp final : public ReplicationCoordinator {

public:
    CryptDReplicationCoordinatorNoOp(ServiceContext* serviceContext);
    ~CryptDReplicationCoordinatorNoOp() = default;

    CryptDReplicationCoordinatorNoOp(CryptDReplicationCoordinatorNoOp&) = delete;
    CryptDReplicationCoordinatorNoOp& operator=(CryptDReplicationCoordinatorNoOp&) = delete;
