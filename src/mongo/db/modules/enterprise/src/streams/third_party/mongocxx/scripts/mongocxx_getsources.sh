...
PLATFORM_DIR="$DIST_DIR/platform/$platformName"
...
TEMP_BSONCXX_PLATFORM_DIR=/tmp/existing/bsoncxx/$platformName
rm -rf $TEMP_BSONCXX_PLATFORM_DIR
mkdir -p $TEMP_BSONCXX_PLATFORM_DIR
cp -r $PLATFORM_DIR/bsoncxx $TEMP_BSONCXX_PLATFORM_DIR

TEMP_MONGOCXX_PLATFORM_DIR=/tmp/existing/mongocxx/$platformName
rm -rf $TEMP_MONGOCXX_PLATFORM_DIR
mkdir -p $TEMP_MONGOCXX_PLATFORM_DIR
cp -r $PLATFORM_DIR/mongocxx $TEMP_MONGOCXX_PLATFORM_DIR
...
mkdir -p $PLATFORM_DIR/mongoc
cp cmake-build/src/libmongoc/src/mongoc/mongoc-config.h $PLATFORM_DIR/mongoc

# Copy the mongoc licences
cp THIRD_PARTY_NOTICES $DIST_DIR/mongoc
cp COPYING $DIST_DIR/mongoc

# Download the 3.7.1 release of mongocxx
cd $TEMP_DIR
...
mkdir -p $PLATFORM_DIR/bsoncxx/config/private
cp -r $TEMP_BSONCXX_PLATFORM_DIR/bsoncxx/config/* $PLATFORM_DIR/bsoncxx/config
...
mkdir -p $PLATFORM_DIR/mongocxx/config/private
cp -r $TEMP_MONGOCXX_PLATFORM_DIR/mongocxx/config/* $PLATFORM_DIR/mongocxx/config
