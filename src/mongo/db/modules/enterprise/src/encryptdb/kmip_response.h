...
namespace kmip {
    ...
    public:
         ...
         */
        uint32_t getOpType() const { return _opType; }

        /**
         * Gets the KMIP result reason and result msg for the parsed response
         */
        uint32_t getResultStatus() const { return _resultStatus; }
        uint32_t getResultReason() const { return _resultReason; }
        std::string getResultMsg() const { return _resultMsg; }

        /**
         * Gets the symmetric key from the parsed message
         */
        std::unique_ptr<SymmetricKey> getSymmetricKey() { return std::move(_symmetricKey); }
    ...
    private:
        ...
        StatusWith<std::unique_ptr<SymmetricKey>> _parseSymmetricKeyPayload(
