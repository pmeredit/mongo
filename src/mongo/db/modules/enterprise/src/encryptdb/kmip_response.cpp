...
namespace kmip {
        ...
        do {
            ...
            if (!adv.isOK()) {
                ...
                return Status(ErrorCodes::FailedToParse, str::stream() <<
                        "KMIP response message invalid, looking for " << tagName <<
                        " tag. " << adv.reason());
            }
    ...
    StatusWith<std::tuple<uint32_t,uint32_t>> KMIPResponse::_parseProtocolVersion(
        ...
        StatusWith<size_t> swTag = _parseTag(cdrc, protocolVersionTag,
                                               ItemType::structure, "protocol version");
        if (!swTag.isOK()) {
            return swTag.getStatus();
        }

        auto swMajor = _parseInteger(cdrc, protocolVersionMajorTag,
                                     ItemType::integer, "protocol version major");
        if (!swMajor.isOK()) {
            return swMajor.getStatus();
        }

        auto swMinor = _parseInteger(cdrc, protocolVersionMinorTag,
                                     ItemType::integer, "protocol version minor");
        if (!swMinor.isOK()) {
    ...
    StatusWith<time_t> KMIPResponse::_parseTimeStamp(ConstDataRangeCursor* cdrc) {
        StatusWith<size_t> swTag = _parseTag(cdrc, timeStampTag,
                                               ItemType::dateTime, "timestamp");
        if (!swTag.isOK()){
            return swTag.getStatus();
        }
    ...
    Status KMIPResponse::_parseResponseHeader(ConstDataRangeCursor* cdrc) {
        StatusWith<size_t> swTag = _parseTag(cdrc, responseHeaderTag,
                                               ItemType::structure, "response header");
        if (!swTag.isOK()){
            return swTag.getStatus();
        }
        ...
        auto swBatchCount = _parseInteger(cdrc, batchCountTag,
                                          ItemType::integer, "batch count");
    ...
    StatusWith<std::string> KMIPResponse::_parseUIDPayload(ConstDataRangeCursor* cdrc) {
        auto swObjectType = _parseInteger(cdrc, objectTypeTag,
                                          ItemType::enumeration, "object type");
    ...
    StatusWith<std::unique_ptr<SymmetricKey>> KMIPResponse::_parseSymmetricKeyPayload(
        ...
        StatusWith<size_t> swTag = _parseTag(cdrc, symmetricKeyTag,
                                               ItemType::structure, "symmetric key");
        if (!swTag.isOK()){
            return swTag.getStatus();
        }
        ...
        auto swKeyFormat = _parseInteger(cdrc, keyFormatTypeTag,
                                         ItemType::enumeration, "key format type");
        if (!swKeyFormat.isOK()){
        ...
        if (keyFormat != keyFormatRaw && keyFormat != keyFormatSymmetricKey) {
            return Status(ErrorCodes::FailedToParse, str::stream() <<
                          "Unexpected key format " << keyFormat);
        }
        ...
        auto swAlgorithm = _parseInteger(cdrc, cryptographicAlgorithmTag,
                                         ItemType::enumeration, "cryptographic algorithm");
        if (!swAlgorithm.isOK()) {
            return swAlgorithm.getStatus();
        }
        uint32_t algorithm = static_cast<uint32_t>(swAlgorithm.getValue());

        auto swKeySize = _parseInteger(cdrc, cryptographicLengthTag,
                                       ItemType::integer, "cryptographic length");
        if (!swKeySize.isOK()) {
        ...
        return stdx::make_unique<SymmetricKey>(key, keySize, algorithm);
    }

    Status KMIPResponse::_parseBatchItem(ConstDataRangeCursor* cdrc) {
        StatusWith<size_t> swTag = _parseTag(cdrc, batchItemTag,
                                               ItemType::structure, "batch item");
        if (!swTag.isOK()){
            return swTag.getStatus();
        }
        ...
        auto swResultStatus = _parseInteger(cdrc, resultStatusTag,
                                            ItemType::enumeration, "result status");
        ...
        switch(_opType) {
            ...
            default:
                return Status(ErrorCodes::FailedToParse, str::stream() <<
                    "Response message was malformed: unknown operation type " << _opType);
        }
    ...
    Status KMIPResponse::_parseFailure(ConstDataRangeCursor* cdrc) {
        auto swResultReason = _parseInteger(cdrc, resultReasonTag,
                                            ItemType::enumeration, "result reason");
    ...
    Status KMIPResponse::_parseResponse(ConstDataRangeCursor* cdrc) {
        StatusWith<size_t> swTag = _parseTag(cdrc, responseMessageTag,
                                               ItemType::structure, "response message");
        if (!swTag.isOK()){
            return swTag.getStatus();
        }
