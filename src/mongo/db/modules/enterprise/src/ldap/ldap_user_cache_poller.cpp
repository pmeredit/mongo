...
MONGO_FAIL_POINT_DEFINE(failToRefreshLDAPUsers);
...
Date_t LDAPUserCachePoller::refreshExternalEntries(Client* client, Date_t lastSuccessfulRefresh) {
    ...
    // The failToRefreshLDAPUsers failpoint is used to simulate failed refresh calls without going
    // over the network.
    Status status = Status::OK();
    auto opCtx = client->makeOperationContext();
    if (MONGO_unlikely(failToRefreshLDAPUsers.shouldFail())) {
        status = {ErrorCodes::OperationFailed, str::stream() << "Operation timed out"};
    } else {
        status = AuthorizationManager::get(opCtx->getServiceContext())
                     ->refreshExternalUsers(opCtx.get());
    }
...
void LDAPUserCachePoller::invalidateExternalEntries(OperationContext* opCtx) {
    try {
        AuthorizationManager::get(opCtx->getServiceContext())
            ->invalidateUsersFromDB(opCtx, "$external"_sd);
