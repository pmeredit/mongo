...
namespace {
    ...
    TEST(AuditMetadata, ReadFromMetadata) {
        ...
        }
        {
            ...
            auto am = checkParse(BSON("$audit" << BSON("$impersonatedUsers" << BSONArray() <<
        ...
        }
        {
            ...
            auto am = checkParse(BSON("$audit" << BSON("$impersonatedRoles" << BSONArray() <<
        ...
        }
        {
            ...
            auto metadata = BSON("$audit"
                                 << BSON("$impersonatedUsers"
                                         << BSON_ARRAY(BSON("user" << "foo" <<
                                                            "db" << "bar") <<
                                                       BSON("user" << "baz" <<
                                                            "db" << "garply"))
                                         << "$impersonatedRoles"
        ...
        }
        {
            ...
            auto metadata = BSON("$audit"
                                 << BSON("$impersonatedUsers" << BSONArray() <<
                                         "$impersonatedRoles"
                                         << BSON_ARRAY(BSON("role" << "r0" <<
                                                            "db" << "db0") <<
                                                       BSON("role" << "r1" <<
                                                            "db" << "db1"))));
            auto am = checkParse(metadata);
    ...
    TEST(AuditMetadata, ReadFromInvalidMetadata) {
        ...
        {
            // users not array
            checkParseFails(BSON("$audit" << BSON("$impersonatedUsers" << 3 <<
                                                  "$impersonatedRoles" << BSONArray())));
        }
        {
            // roles not array
        ...
        {
            // invalid users
            checkParseFails(BSON("$audit" << BSON("$impersonatedUsers" << BSON_ARRAY(2 << 3) <<
                                                  "$impersonatedRoles" << BSONArray())));
        }
        {
            // invalid roles
            checkParseFails(BSON("$audit" << BSON("$impersonatedUsers" << BSONArray() <<
                                                  "$impersonatedRoles" << BSON_ARRAY(1 << 2))));
        }
        {
            // extra keys
            checkParseFails(BSON("$audit" << BSON("$impersonatedUsers" << BSONArray() <<
    ...
    void checkUpconvert(const BSONObj& legacyCommand,
        ...
        ASSERT_OK(AuditMetadata::upconvert(legacyCommand,
                                           0,
    ...
    TEST(AuditMetadata, Upconvert) {
        {
            ...
            checkUpconvert(BSON("ping" << 1 <<
                                ...
                                "impersonatedRoles" << BSONArray()),
                           BSON("ping" << 1),
                           BSON("$audit" << BSON("$impersonatedUsers" << BSONArray() <<
                                                 "$impersonatedRoles" << BSONArray())));
        }
        {
            // no impersonators
            checkUpconvert(BSON("ping" << "pong" << "bing" << "bong"),
                           BSON("ping" << "pong" << "bing" << "bong"),
                           BSONObj());
        ...
        }
        {
            ...
            checkUpconvert(BSON("ping" << 1 <<
                                "impersonatedUsers" << BSON_ARRAY(BSON("user" << "u0" <<
                                                                       "db" << "db0")) <<
                                "impersonatedRoles" << BSON_ARRAY(BSON("role" << "r0" <<
                                                                       "db" << "r0"))),
                           BSON("ping" << 1),
                           BSON("$audit" << BSON("$impersonatedUsers" <<
                                                 BSON_ARRAY(BSON("user" << "u0" <<
                                                                 "db" << "db0")) <<
                                                 "$impersonatedRoles" <<
                                                 BSON_ARRAY(BSON("role" << "r0" <<
    ...
    void checkUpconvertFails(const BSONObj& legacyCommand) {
        BSONObjBuilder upconvertedCommandBob;
        BSONObjBuilder upconvertedMetadataBob;
        ASSERT_NOT_OK(AuditMetadata::upconvert(legacyCommand,
                                               0,
                                               &upconvertedCommandBob,
                                               &upconvertedMetadataBob));
    }
    ...
    TEST(AuditMetadata, UpconvertInvalid) {
        {
            // users wrong type
            checkUpconvertFails(BSON("foo" << "bar"
                                     << "impersonatedUsers" << 3
                                     << "impersonatedRoles" << BSONArray()));
        }
        {
    ...
    void checkDownconvert(const BSONObj& command,
        ...
        int ignored;
        ASSERT_OK(AuditMetadata::downconvert(command,
                                             metadata,
    ...
    TEST(AuditMetadata, Downconvert) {
        ...
        }
        {
            checkDownconvert(BSON("ping" << 1),
                             BSON("$audit" <<
                                  BSON("$impersonatedUsers" << BSON_ARRAY(BSON("user" << "u0" <<
                                                                               "db" << "db0") <<
                                                                          BSON("user" << "u1" <<
                                                                               "db" << "db1")) <<
                                       "$impersonatedRoles" << BSON_ARRAY(BSON("role" << "r0" <<
