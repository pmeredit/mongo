...
TEST(KMIPRequestGeneration, DiscoverVersions) {
    ...
    std::vector<uint8_t> expectedOutput = buildExpectedOutput(0x1E, requestSizes, payload);
...
TEST(KMIPRequestGeneration, Activate) {
    ...
    std::vector<uint8_t> payload = {0x42, 0x00, 0x94, 0x07, 0x00, 0x00, 0x00, 0x03,
                                    0x31, 0x32, 0x33, 0x00, 0x00, 0x00, 0x00, 0x00};
    // clang-format on
    std::vector<uint8_t> expectedOutput = buildExpectedOutput(0x12, requestSizes, payload);
...
TEST(KMIPRequestGeneration, GetAttributes) {
    ...
    // clang-format on
    std::vector<uint8_t> expectedOutput = buildExpectedOutput(0x0B, requestSizes, payload);
